...
class DebugBase:
    ...
    def __profile_call(
        self, code: CodeType, _instruction_offset: int, callable: object, _arg0: object
    ):
        """
        ...
        @param _instruction_offset where the call is performed next
        @type int
        @param callable function / method which will be called
        @type object
    ...
    def __profile_c_return(
        ...
        @param code current code object
        @type CodeType
        @param _instruction_offset DESCRIPTION
        @type int
        @param callable DESCRIPTION
        @type object
        @param _arg0 DESCRIPTION
        @type object
        @return monitoring.DISABLE if further calls from this code object should be
            suppressed
        @rtype None | monitoring.DISABLE
        ...
        try:
            ...
            # Call into C-code
            f_code = Code('built-in', f' {callable}')
            line_number = -1
    ...
    def _has_breakpoint(self, code: CodeType):
        ...
        try:
            ...
            if bp_lines:
                line_numbers = set(list(zip(*code.co_lines()))[2])
    ...
    def step(self, step_into):
        ...
        with contextlib.suppress(ValueError):
            # Raised if debugging stopped while in a BP or tiggered by a late thread
            monitoring.set_local_events(
                monitoring.DEBUGGER_ID, self.currentFrame.f_code,
    ...
    def stepOut(self):
        ...
        if step_over_frame.f_code.co_filename in self.bootstrap_files:
            ...
            with contextlib.suppress(ValueError):
                # Raised if debugging stopped while in a BP or tiggered by a late thread
                monitoring.set_local_events(
                    monitoring.DEBUGGER_ID, step_over_code,
                    events.LINE | events.PY_RETURN | events.PY_YIELD
                )
    ...
    def __event_poll_timer(self):
        ...
        """
        self._dbgClient.pollTimerThreadId = _thread.get_ident()
    ...
    def setCurrentThread(self, threadId):
        ...
        self.unlockClient()
        time.sleep(0.5 * sys.getswitchinterval())  # Continue with next thread
        self.eventExit = True  # Now exit this event loop

    def getStack(self, frame=None):
        """
    ...
    def user_exception(self, excinfo, unhandled=False):
        ...
        self._dbgClient.sendException(exctypetxt, str(excval), stack, self.name)
        self._dbgClient.exec_threads.append(thread_id)
